...
def run_plain(mode: str = "v2_plain_baseline") -> Path:
    env = load_demo_env()
    ...
    # Second pass (this is the inefficiency)
    response2 = model.invoke([
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user", "content": user_prompt},
        {"role": "assistant", "content": response1.content},
        {"role": "user", "content": "Double-check and refine your answer."},
    ])

    output = response2.content if isinstance(response2.content, str) else str(response2.content)
